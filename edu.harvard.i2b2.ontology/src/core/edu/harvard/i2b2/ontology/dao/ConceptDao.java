...
import edu.harvard.i2b2.ontology.datavo.vdo.ConceptCountsType;
import edu.harvard.i2b2.ontology.datavo.vdo.ConceptType;
import edu.harvard.i2b2.ontology.datavo.vdo.CountType;
import edu.harvard.i2b2.ontology.datavo.vdo.DomainCountsType;
...
public class ConceptDao extends JdbcDaoSupport {
	...
	final static String CAT_CORE = " c_hlevel, c_fullname, c_name, c_synonym_cd, c_visualattributes, c_totalnum, c_basecode, c_facttablecolumn, c_dimtablename, c_columnname, c_columndatatype, c_operator, c_dimcode, c_tooltip, valuetype_cd, domain_name, on_parent ";
	final static String CAT_DEFAULT = " c_fullname, c_name ";
	final static String CAT_LIMITED =  " c_hlevel, c_fullname, c_name, c_synonym_cd, c_visualattributes, c_totalnum, c_basecode, c_tooltip, valuetype_cd, domain_name, on_parent ";
		...
		ParameterizedRowMapper<ConceptType> mapper = new ParameterizedRowMapper<ConceptType>() {
			...
			public ConceptType mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				if(returnType.getType().equals("limited")) {
					...
					child.setDomainName(rs.getString("domain_name"));
					child.setOnParent(rs.getString("on_parent"));
//					child.setOwner(rs.getString("owner"));
				}
				else if(returnType.getType().equals("core")) {
					child.setBasecode(rs.getString("c_basecode"));
					child.setLevel(rs.getInt("c_hlevel"));
...
//					child.setOwner(rs.getString("owner"));
	...
	public List findChildrenByParent(final GetChildrenDataMessage childrenMsg, ProjectType projectInfo, DBInfoType dbInfo) throws I2B2DAOException, I2B2Exception, JAXBUtilException{
		...
		if (!protectedAccess){
			String tableSql = "select distinct(c_table_name) from " + metadataSchema + "table_access where c_table_cd = ? and c_protected_access = ? and on_parent = 'Y' ";
			...
			String tableSql = "select distinct(c_table_name) from " + metadataSchema + "table_access where c_table_cd = ? and on_parent = 'Y' ";
			try {
		...
		//Get Counts per concept per domain.
		if(queryResult.size() > 0){
			Iterator<ConceptType>  it2 = queryResult.iterator();
			while (it2.hasNext()){
				ConceptType concept = (ConceptType)it2.next();			
				getConceptCounts(concept, metadataSchema, tableName, obfuscatedUserFlag);
//				log.info("Added to concept before sending to client: " + concept.getConceptCounts().getDomainCounts().size());
			}
		}
		return queryResult;
	...
	public void getConceptCounts(ConceptType concept, String metadataSchema, String tableName, boolean obfuscatedUserFlag){
		// first get the set of domains

		String domainSql = " select distinct(domain_name) from " + metadataSchema+ tableName + "_counts" ;
	//	log.info(domainSql);
		
		List<String> domainResult = null;
		try {
			domainResult = jt.query(domainSql, domainMapper );
		} catch (Exception e) {
			log.error(e.getMessage());
	//		throw new I2B2DAOException("Database Error");
		}
		
	ConceptCountsType allCounts = new ConceptCountsType();
	//	DomainCountsType	domainCounts = new DomainCountsType();
		
		
		if(domainResult.size() > 0){
			Iterator<String>  it2 = domainResult.iterator();
			while (it2.hasNext()){
				String domainName = (String)it2.next();
				
				String countsSql = "select count_type, c_totalnum from " + metadataSchema+ tableName + "_counts " 
					+ " where domain_name = ? and c_fullname = ?";
				
		//		log.info(countsSql + " " + domainName + " " + StringUtil.getPath(concept.getKey()));
				ParameterizedRowMapper<CountType> domainCountsMapper = getDomainCountsMapper(obfuscatedUserFlag);

				List<CountType> countsResult = null;
				try {
					countsResult = jt.query(countsSql, domainCountsMapper, domainName, StringUtil.getPath(concept.getKey()) );
				} catch (DataAccessException e) {
					// TODO Auto-generated catch block
					log.error(e.getMessage());
				}
				
				if(countsResult.size() > 0){
					DomainCountsType domainCounts = new DomainCountsType();
				
					Iterator<CountType>  it3 = countsResult.iterator();
					while (it3.hasNext()){
						CountType count = (CountType)it3.next();
						domainCounts.getCounts().add(count);
						
					}
					domainCounts.setDomainName(domainName);
			//		log.info("DomainCount" + domainCounts.getDomainName() + " " +
			//				domainCounts.getCounts().get(1).getCount() + " " +
			//				domainCounts.getCounts().get(1).getType());
					
					allCounts.getDomainCounts().add(domainCounts);
				
			//		log.info("ADDED domain count to allCounts " + domainName);
							
				}	
			}
			
		}
		concept.setConceptCounts(allCounts);
//		log.info("Number of concept domainCounts: " + concept.getConceptCounts().getDomainCounts().size());
		
		
/*		if(allCounts.getDomainCounts().size() > 0){
			Iterator<DomainCountsType>  itAllCounts = allCounts.getDomainCounts().iterator();
			while (itAllCounts.hasNext()){
				DomainCountsType dCounts = (DomainCountsType)itAllCounts.next();
				

				log.info("added all counts to concept's count for " + StringUtil.getPath(concept.getKey()));
			}
		}
		*/
	...
	ParameterizedRowMapper<String> domainMapper = new ParameterizedRowMapper<String>() {
		public String mapRow(ResultSet rs, int rowNum) throws SQLException {
	...
	private ParameterizedRowMapper<CountType> getDomainCountsMapper(final boolean ofuscatedUserFlag){

		ParameterizedRowMapper<CountType> domainCountsMapper = new ParameterizedRowMapper<CountType>() {
			public CountType mapRow(ResultSet rs, int rowNum) throws SQLException {
				CountType count= new CountType();
				count.setType(rs.getString("count_type"));
				Integer totalNumValue = rs.getInt("c_totalnum");
				boolean nullFlag = rs.wasNull();

				if ( ofuscatedUserFlag == false && nullFlag == false) { 
					count.setCount(totalNumValue);
				}
				return count;
			} 
		};
		return domainCountsMapper;
	}
	...
	public List findNameInfo(final VocabRequestType vocabType, ProjectType projectInfo, DBInfoType dbInfo) throws DataAccessException, I2B2Exception{
		...
		if (!protectedAccess){
			String tableSql = "select distinct(c_table_name), c_fullname from " + metadataSchema + "table_access where c_table_cd = ? and c_protected_access = ? and on_parent = 'Y' ";
			try {
			...
			String tableSql = "select distinct(c_table_name), c_fullname from " + metadataSchema + "table_access where c_table_cd = ? and on_parent = 'Y' ";
			try {
	...
	public List findCodeInfo(final VocabRequestType vocabType, ProjectType projectInfo, DBInfoType dbInfo) throws DataAccessException, I2B2Exception{
		...
		if (!protectedAccess){
			String tableSql = "select distinct(c_table_name) from " + metadataSchema + "table_access where c_protected_access = ? and on_parent = 'Y'";
			//			log.info(tableSql);
			try {
			...
			String tableSql = "select distinct(c_table_name) from " + metadataSchema + "table_access where on_parent = 'Y'";
			try {
		...
		if(tableNames != null){
			...
			String tableCdSql = ", (select distinct(c_table_cd) from "+ metadataSchema + "TABLE_ACCESS where on_parent = 'Y' and c_table_name = '"+  table+ "') as tableCd"; 
			if(dbType.toUpperCase().equals("SQLSERVER"))
				tableCdSql = ", (select top 1(c_table_cd) from "+ metadataSchema + "TABLE_ACCESS where on_parent = 'Y' and c_table_name = '"+  table+ "') as tableCd"; 
			else if (dbType.toUpperCase().equals("ORACLE"))
				tableCdSql = ", (select c_table_cd from "+ metadataSchema + "TABLE_ACCESS where on_parent = 'Y' and c_table_name = '"+  table+ "' and rownum <= 1) as tableCd"; 
			else if(dbType.toUpperCase().equals("POSTGRESQL"))
				tableCdSql = ", (select c_table_cd from "+ metadataSchema + "TABLE_ACCESS where on_parent = 'Y' and c_table_name = '"+  table+ "' limit 1) as tableCd"; 
			codeInfoSql = "select " + parameters + tableCdSql + " from " + metadataSchema + table + whereClause	+ hidden + synonym;;
			while(itTn.hasNext()){		
				table = (String)itTn.next();
				// the following (distinct) doesnt work for a flattened hierarchy but is left for
				//  dbs other than sqlserver or oracle.    [c_table_cd is needed for key]
				tableCdSql = ", (select distinct(c_table_cd) from "+ metadataSchema + "TABLE_ACCESS where on_parent = 'Y' and c_table_name = '"+  table+ "') as tableCd"; 
				if(dbType.toUpperCase().equals("SQLSERVER"))
					tableCdSql = ", (select top 1(c_table_cd) from "+ metadataSchema + "TABLE_ACCESS where on_parent = 'Y' and c_table_name = '"+  table+ "') as tableCd"; 
				else if (dbType.toUpperCase().equals("ORACLE"))
					tableCdSql = ", (select c_table_cd from "+ metadataSchema + "TABLE_ACCESS where on_parent = 'Y' and c_table_name = '"+  table+ "' and rownum <= 1) as tableCd"; 
