...
public class CallPMUtil {
	...
	public CellDataType getCellForProject(CellDatasType cellsData, String projectPath, String id)
	{
		//CellDataType result = null;
		
		
		
		for (CellDataType cellData: cellsData.getCellData())
		{
			if ((cellData.getProjectPath()).equals(projectPath) && (cellData.getId().toLowerCase().equals(id.toLowerCase())))
				return cellData;
	...
	public ConfiguresType callGetAllHive() throws AxisFault, I2B2Exception {
		RequestMessageType requestMessageType = getI2B2RequestMessageAllHive();
		OMElement requestElement = null;
		ConfiguresType projectType = null;
		try {
			requestElement = buildOMElement(requestMessageType);
			log.debug("CRC PM call's request xml " + requestElement);
			OMElement response = getServiceClient().sendReceive(requestElement);
			projectType = getHiveFromResponse(response.toString());
		} catch (XMLStreamException e) {
			e.printStackTrace();
			throw new I2B2Exception("" + StackTraceUtil.getStackTrace(e));
		} catch (JAXBUtilException e) {
	...
	public ConfigureType callUserConfigure() throws AxisFault, I2B2Exception {
		RequestMessageType requestMessageType = getI2B2RequestMessage();
		OMElement requestElement = null;
		ConfigureType projectType = null;
		try {
			requestElement = buildOMElement(requestMessageType);
			log.debug("CRC PM call's request xml " + requestElement);
			OMElement response = getServiceClient().sendReceive(requestElement);
			projectType = getUserConfigureFromResponse(response.toString());
		} catch (XMLStreamException e) {
			e.printStackTrace();
			throw new I2B2Exception("" + StackTraceUtil.getStackTrace(e));
		} catch (JAXBUtilException e) {
	...
	private ConfigureType getUserConfigureFromResponse(String responseXml)
			throws JAXBUtilException, I2B2Exception {
		JAXBElement responseJaxb = CRCJAXBUtil.getJAXBUtil()
				.unMashallFromString(responseXml);
		ResponseMessageType pmRespMessageType = (ResponseMessageType) responseJaxb
	...
	private ConfiguresType getHiveFromResponse(String responseXml)
			throws JAXBUtilException, I2B2Exception {
		JAXBElement responseJaxb = CRCJAXBUtil.getJAXBUtil()
				.unMashallFromString(responseXml);
		ResponseMessageType pmRespMessageType = (ResponseMessageType) responseJaxb
		...
		JAXBUnWrapHelper helper = new JAXBUnWrapHelper();
		ConfiguresType configureType = (ConfiguresType) helper.getObjectByClass(
				pmRespMessageType.getMessageBody().getAny(),
				ConfiguresType.class);
	...
	private RequestMessageType getI2B2RequestMessageAllHive() {
		...
		bodyType.getAny().add(of.createGetAllHive(""));
